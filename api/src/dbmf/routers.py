...
async def get_dbmf_tickers(db: Session = Depends(get_db)) -> dict[str, dict[str, str]]:
    try:
        return operations.extract_tickers_from_db(db)
    except exceptions.NoTableFoundException:
        raise HTTPException(status_code=404, detail="Tickers' table not found")
    except exceptions.NoDbmfTickerFoundException:
        raise HTTPException(status_code=200, detail="Tickers' table empty")
...
async def get_stats(
    ...
    try:
        ...
        raise HTTPException(
            status_code=404, detail=f"No {ticker.upper()} postion status table found"
        )
